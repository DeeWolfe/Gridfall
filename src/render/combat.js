...
import {frameGateText} from '../rules/frames.js';
import {costOf, gearOf, vetOf, leadOf, leadBan, cardName} from '../save/progression.js';
...
/** Leaving combat: back to the map, the run's map, or mode select. */
export function leaveCombat() {
  ...
  const bossDown = G && G.over && G.type === 'boss' && G.result && G.result.cleared && G.boss
    ? G.boss.k : null;
  const {wasEndless, wasDaily, wasRun} = abortMission();
...
const abortStakes = () => (G.run
  ? 'A Deep Descent has no second attempt at a layer. Walk out and the run closes where it stands — ' +
...
function drawSel() {
  ...
  if (sel) {
    const k = POOL[sel];
    const g = gearOf(sel);
    const dmg = (k.dmg || 0) + (g && g.dmg ? g.dmg : 0);
    // A gated card (a second Frame, gear whose machine is absent) lights
    // nothing — say why instead of leaving the player tapping dead tiles.
    const gate = frameGateText(sel);
    const anchors = validTiles(sel).length;
    el.innerHTML = `<div class="selhead"><b style="color:var(--green)">${k.n}</b>
      ...
      <div class="selgrid">
        ...
        <div><span>Deploy</span><b>${k.frameGear ? 'Onto its Frame'
    : k.drop ? 'Any tile'
    ...
    ? (k.frameGear ? 'Tap the Frame to fit it' : 'Tap a lit tile to deploy')
    : (gate || 'No legal tile to deploy on.')}</div>
...
export function drawHand() {
  ...
  G.hand.forEach((cid, index) => {
    ...
    const banned = leadBan(cid) || frameGateText(cid);
    const unaffordable = cost > G.dp || G.over || !!banned;
    const g = gearOf(cid);
    const v = vetOf(cid);

    const el = document.createElement('div');
    el.className = `hc t-${k.t} v${v.t}` + (k.chassis === 'proto' ? ' proto' : '') + (unaffordable ? ' poor' : '') + (sel === cid ? ' sel' : '');
