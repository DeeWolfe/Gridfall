...
// 双 "pair" for the two-body fireteam. All 46 are distinct, which is also
...
const KANJI = {
  ...
  hell: '焔', plasma: '光', exo: '鎧', hecate: '砲',
};
...
/** Full-bleed seal face; `accent` is the tier or veterancy colour. */
export function cardPortrait(id, accent) {
  const k = KANJI[id];
  if (!k) return '';
  const c = accentFor(id, accent);
  const heavy = POOL[id] && POOL[id].t === 'special';
  const name = ((POOL[id] && POOL[id].n) || id).toUpperCase().slice(0, 12);
  return `<svg class="artfill" viewBox="0 0 100 140" preserveAspectRatio="xMidYMid slice">
    ...
    ${enso(c, 50, 58, heavy)}
    <text x="50" y="76" text-anchor="middle" font-size="52" fill="${INK}"
      font-family="${KFONT}">${k}</text>
    <text x="10" y="130" font-size="7" fill="${c}" opacity=".85" letter-spacing="2"
      font-family="ui-monospace,monospace">${name}</text>
