...
function statRows(id) {
  ...
  return [
    ...
    (k.sight || (g && g.sight)) ? ['Sight', `Sees ${(k.sight || 1) + ((g && g.sight) || 0)} cells in the fog`] : null,
    (g && g.kitrack) ? ['Kit rack', 'Carries two armour abilities at once'] : null,
    ...
    k.berserk ? ['Damage', 'Scales with the machine’s own wounds — +1 below three-quarters hull, +2 below half, +3 below a quarter'] : null,
    ...
    (k.regen || (g && g.regen)) ? ['Shield', 'Regenerates each turn'] : null,
    k.attach === 'shield' ? ['Charges', 'Two — each eats one blow whole'] : null,
    k.parapet ? ['Parapet', 'Friendly direct fire passes over it'] : null,
    k.formation === 'column' ? ['Formation', 'Bodies file down the column, never sideways'] : null,
    k.twin ? ['Rate of fire', 'Twice a turn'] : null,
    k.burnLane ? ['Field', `Every hostile in its lane burns for ${k.burnLane} each enemy phase`] : null,
    k.hymn ? ['Song', `Hostiles within two cells strike ${k.hymn} softer`] : null,
    k.dynamo ? ['Supply', `+${k.dynamo} deploy point every turn it stands`] : null,
    (k.frameGear && k.blocker) ? ['Kit trait', 'The body becomes a blocker'] : null,
