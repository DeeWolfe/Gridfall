 ...
 * and tapping End turn: measured over 200 idle runs the median is 5 and 97% get
 * there, so the mode paid a pack for sitting still. No idle run reached 10, so
...
export function launchSpec(nd) {
  ...
  setG({
    ...
    gridCharge: Array(LANES).fill(0), event: null, eventNext: null,
...
function settleOnslaught() {
  ...
  for (let i = 0; i < Math.floor(G.turn / ONSLAUGHT_PACK_WAVES); i++) {
    queuePack('standard', `Onslaught · wave ${(i + 1) * ONSLAUGHT_PACK_WAVES}`);
  }
